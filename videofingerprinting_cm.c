...
int main(int argc, char *argv[]) {
  ...
  sprintf(table_query,"create table '%s' (key INTEGER PRIMARY KEY, s_end FLOAT, t_diff FLOAT, luma INTEGER, luma_diff INTEGER);",filename);
...
int makeIndexes(int *shortArray, sqlite3* handle, char *filename, int threshold, int size, double fps) {
  ...
  for (aux = 0 ; aux < size ; aux++) {
	...
	if ( (shortArray[aux] < (first-thresh) || shortArray[aux] > (first+thresh)) ) {
      ...
      memset(insert_query, 0, 500000);
      sprintf(insert_query, "insert into '%s' values (%d,%f,%f,%d,%d)",filename,local_aux,aux/fps,(aux/fps)-lastTime,first,lastLuma-first);
  ...
  // Last entry
  memset(insert_query, 0, 500000);
  sprintf(insert_query, "insert into '%s' values (%d,%f,%f,%d,%d)",filename,local_aux+1,aux/fps,(aux/fps)-lastTime,-1,-1);
  retval = sqlite3_exec(handle,insert_query,0,0,0);
