...
if sqlite
  ...
  db_temp = SQLite3::Database.new( "/home/gsc/test_suj_utube_temp.db" ) if linux && (( sqlite || sqlite_import ) && sqlite_import_sql)
  db = SQLite3::Database.new( "test.db" ) if !linux
  db = SQLite3::Database.new( "/home/gsc/test_suj_newtable_#{@@param}_utube_lut_li.db" ) if linux && ( sqlite || sqlite_import || sqlite_import_sql )
  db = SQLite3::Database.new( "/home/gsc/test_suj_3tablepermovie_#{@@param}_utube_lut_li.db" ) if linux && threeTables && ( sqlite || sqlite_import || sqlite_import_sql )
  #
...
videoArray.each do |video|
  ...
  #Exception control
  #raise RuntimeError, "No log file for #{video}! Did the analysis complete successfuly? Maybe the data is in the temp database? (also use the -sqlite_import_sql too)" if File.exists?(path + video + ".log2") == false && !sqlite_import_sql
	...
	if !threeTables
	  ...
	  begin
		...
		(0..keys.size-2).each do |x| 
		  ...
		  #Luis' idea
		  db.execute( "update hashluma set movies = \"#{prev.to_s+mid.to_s+":"+keys[x].to_s+"-"+keys[x+1].to_s+","}\" where avg_range = \"#{(avgLuma/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
		  ...
		  #
		  #
		  prev = db.execute( "select * from hashcu where avg_range = \"#{(avgChromaU/100.0).round}\"" )
		  if !prev.empty?
			prev = prev[0][1]
		  else
		    prev = "".to_s if prev.empty?
		  ...
		  #Luis' idea
		  db.execute( "update hashcu set movies = \"#{prev.to_s+mid.to_s+":"+keys[x].to_s+"-"+keys[x+1].to_s+","}\" where avg_range = \"#{(avgChromaU/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
		  ...
		  #
		  #
		  prev = db.execute( "select * from hashcv where avg_range = \"#{(avgChromaV/100.0).round}\"" )
		  if !prev.empty?
			prev = prev[0][1]
		  else
		    prev = "".to_s if prev.empty?
		  ...
		  #Luis' idea
		  db.execute( "update hashcv set movies = \"#{prev.to_s+mid.to_s+":"+keys[x].to_s+"-"+keys[x+1].to_s+","}\" where avg_range = \"#{(avgChromaV/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
	...
	if threeTables
	  ...
	  begin
		...
		(0..keys.size-2).each do |x| 
		  ...
		  #append to "hashtables"
		  prev = db.execute( "select * from hashluma where avg_range = \"#{(avg/100.0).round}\"" )
		  if !prev.empty?
			prev = prev[0][1]
		  else
		    prev = "".to_s if prev.empty?
		  ...
		  #Luis' idea
		  db.execute( "update hashluma set movies = \"#{prev.to_s+mid.to_s+":"+keys[x].to_s+"-"+keys[x+1].to_s+","}\" where avg_range = \"#{(avg/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
	  ...
	  begin
		...
		(0..keys.size-2).each do |x| 
		  ...
		  #append to "hashtables"
		  prev = db.execute( "select * from hashcu where avg_range = \"#{(avg/100.0).round}\"" )
		  if !prev.empty?
			prev = prev[0][1]
		  else
		    prev = "".to_s if prev.empty?
		  ...
		  #Luis' idea
		  db.execute( "update hashcu set movies = \"#{prev.to_s+mid.to_s+":"+keys[x].to_s+"-"+keys[x+1].to_s+","}\" where avg_range = \"#{(avg/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil        
	  ...
	  begin
		...
		(0..keys.size-2).each do |x| 
		  ...
		  #append to "hashtables"
		  prev = db.execute( "select * from hashcv where avg_range = \"#{(avg/100.0).round}\"" )
		  if !prev.empty?
			prev = prev[0][1]
		  else
		    prev = "".to_s if prev.empty?
		  ...
		  #Luis' idea
		  db.execute( "update hashcv set movies = \"#{prev.to_s+mid.to_s+":"+keys[x].to_s+"-"+keys[x+1].to_s+","}\" where avg_range = \"#{(avg/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
  ...
  if sqlite
	...
	#This ID is the movie ID, it will tell us what the unique ID of the movie is in the global allmovies DB
	if sqlite_import_sql
		mid = db_temp.execute( "select * from allmovies where name = \"#{video}\"")[0][0].to_i
	else
		mid = db.execute( "select * from allmovies where name = \"#{video}\"")[0][0].to_i
	end
    ...
    if !threeTables
	  ...
	  if sqlite_import_sql
        ...
        raise RuntimeError, "Video is not in database! Did you run the -sqlite_import flag beforehand?"
      end
    ...
    if threeTables
      ...
      begin
	  if sqlite_import_sql
		query = db_temp.execute( "select * from \"#{video}_l\"" )
	  else
      ...
      begin
	  if sqlite_import_sql
		query = db_temp.execute( "select * from \"#{video}_cu\"" )
	  else
      ...
      begin
	  if sqlite_import_sql
		query = db_temp.execute( "select * from \"#{video}_cv\"" )
	  else
  ...
  if sourceVideo == video
	...
	norm_q = (searchSeqArrayChromaUIni.uniq.size/searchSeqArrayChromaUIni.size.to_f+searchSeqArrayChromaVIni.uniq.size/searchSeqArrayChromaVIni.size.to_f)/2
	
	print("Normalization q. is #{norm_q} ...(#{searchSeqArrayChromaUIni.uniq.size}/#{searchSeqArrayChromaUIni.size.to_f}+#{searchSeqArrayChromaVIni.uniq.size}/#{searchSeqArrayChromaVIni.size.to_f})/2")
	#if norm_q >= 0.35
		puts(" normalizing")
		shall_we_norm = true
	#else
	#	puts(" NOT normalizing!")
	#	shall_we_norm = false
	#end
	
    #Normalize the CLIP array
    searchSeqArrayLumaIni = norm(searchSeqArrayLumaIni) if shall_we_norm
    searchSeqArrayChromaUIni = norm(searchSeqArrayChromaUIni) if shall_we_norm
    searchSeqArrayChromaVIni = norm(searchSeqArrayChromaVIni) if shall_we_norm
  ...
  if toSearchL.empty?
	...
	#BUG: shold be auxClipArray? Revise logic!
	auxArray.each do |avg|
		...
		qArray = Array.new
		#qArray = db.execute( "select * from hashluma where avg_range between #{((avg[1].to_i/100.0).round)-5} and #{((avg[1].to_i/100.0).round)+5}" )
		qArray = db.execute( "select * from hashluma where avg_range = #{(avg[1].to_i/100.0).round}" )
...
#  aaux.clear
  ...
  if toSearchL[mid] != nil
    #Now we can see the start and end time for each Luma in the MOVIE
    #diffaux = toSearchL[mid].split('-')
...
#		end
		...
		if count >= idxDistTimeLClip.size-1 #&& countL >= idxDistTimeLClip.size-5
		  #p "#{video}: idx=#{x} Count=#{count}/#{idxDistTimeLClip.size-1} CountL=#{countL}/#{idxDistTimeLClip.size-1}"
		  b_val2 = true
		  break
		end
  ...
  (0..(lumaArraySecCluster.keys.size - 2)).each do |idx|
    (lumaArraySecCluster.keys.sort[idx].to_i..(lumaArraySecCluster.keys.sort[idx+1].to_i)-1).each do |x|
	  ...
	  if shall_we_norm
		  ...
		  tempArrayLuma = bound(lumaArraySec,x,bound,@@lengthSec)
		  tempArrayChromaU = bound(chromaUArraySec,x,bound,@@lengthSec)
		  tempArrayChromaV = bound(chromaVArraySec,x,bound,@@lengthSec)
	  end
	  
	  #p chromaVArraySec.size
	  #p tempArrayChromaV.size
	  #p x
	  #p bound
	  #p @@lengthSec
	  #p tempArrayChromaV
	  ...
	  #if taniLuma[x] >= 0.998
		  #p "#{video}:#{x} with values L=#{taniLuma[x]} Cu=#{taniChromaU[x]} Cv=#{taniChromaV[x]}"
		  #p "Luma:"
		  #p searchSeqArrayLuma
		  #p tempArrayLuma
		  #p "ChromaU:"
		  #p searchSeqArrayChromaU
		  #p tempArrayChromaU
		  #p "ChromaV:"
		  #p searchSeqArrayChromaV
		  #p tempArrayChromaV
	  #end
  ...
  (0..(lumaArraySecCluster.keys.size - 2)).each do |idx|
    (lumaArraySecCluster.keys.sort[idx].to_i..(lumaArraySecCluster.keys.sort[idx+1].to_i)-1).each do |x|
	  ...
	  if CM && !shall_we_norm
		...
		if taniLumaBuff.sort[maxSize-1] >= taniThreshLvh && sleepT == -1
			scoreTani += pointvh
		#elsif taniLumaBuff.sort[maxSize-1] >= taniThreshLh && sleepT == -1
		#	scoreTani += pointh
		elsif taniLumaBuff.sort[maxSize-1] >= taniThreshL && sleepT == -1
			scoreTani += point
		end

		if taniCuBuff.sort[maxSize-1] >= taniThreshCvh && sleepT == -1
			scoreTani += pointvh
		#elsif taniCuBuff.sort[maxSize-1] >= taniThreshCh && sleepT == -1
		#	scoreTani += pointh
		elsif taniCuBuff.sort[maxSize-1] >= taniThreshC && sleepT == -1
			scoreTani += point
		end

		if taniCvBuff.sort[maxSize-1] >= taniThreshCvh && sleepT == -1
			scoreTani += pointvh
		#elsif taniCvBuff.sort[maxSize-1] >= taniThreshCh && sleepT == -1
		#	scoreTani += pointh
		elsif taniCvBuff.sort[maxSize-1] >= taniThreshC && sleepT == -1
			scoreTani += point
		end
	  ...
	  if CM && shall_we_norm
		...
		if taniLumaBuff.sort[maxSize-1] >= taniThreshVH && sleepT == -1
			scoreTani += pointvh
		elsif taniLumaBuff.sort[maxSize-1] >= taniThreshH && sleepT == -1
			scoreTani += pointh
		elsif taniLumaBuff.sort[maxSize-1] >= taniThresh && sleepT == -1
			scoreTani += point
		elsif taniLumaBuff.sort[maxSize-1] <= taniThreshL && sleepT == -1
			scoreTani += pointL
		end

		if ( taniCuBuff.sort[maxSize-1] >= taniThreshVH || taniCvBuff.sort[maxSize-1] >= taniThreshVH ) && sleepT == -1
			scoreTani += pointvh
		elsif ( taniCuBuff.sort[maxSize-1] >= taniThreshH || taniCvBuff.sort[maxSize-1] >= taniThreshH ) && sleepT == -1
			scoreTani += pointh
		elsif ( taniCuBuff.sort[maxSize-1] >= taniThresh || taniCvBuff.sort[maxSize-1] >= taniThresh ) && sleepT == -1
			scoreTani += point
		elsif ( taniCuBuff.sort[maxSize-1] <= taniThreshL || taniCvBuff.sort[maxSize-1] <= taniThreshL ) && sleepT == -1
			scoreTani += pointL
		end

		#if taniCvBuff.sort[maxSize-1] >= taniThreshVH && sleepT == -1
		#	scoreTani += pointvh
		#elsif taniCvBuff.sort[maxSize-1] >= taniThreshH && sleepT == -1
		#	scoreTani += pointh
		#elsif taniCvBuff.sort[maxSize-1] >= taniThresh && sleepT == -1
		#	scoreTani += point
		#elsif taniCvBuff.sort[maxSize-1] <= taniThreshL && sleepT == -1
		#	scoreTani += pointL
		#end

		##
		#
		if (((lumaArraySec.size-1) - x) < @@lengthSec)
			#p "=========== #{(lumaArraySec.size-1)} - #{x}) (#{(((lumaArraySec.size-1)*1.0) - x)}) < #{@@lengthSec}"
			scoreTotal = (score+scoreTani)/(@@lengthSec*1.0/(((lumaArraySec.size-1)*1.0) - x))
		else
			scoreTotal = (score+scoreTani) if (taniLuma.size-1) > @@lengthSec
		end
      ...
      if scoreTotal >= 0.51 && scoreP != scoreTotal && !CM
		print("O") if gui
        finalOut=finalOut+"%5.2f" % (scoreTotal*100.0)+ "% Hit: Video segment found in #{video} [#{realFrame}~#{realFrame+@@lengthSec}]\n"
		puts("%5.2f" % (scoreTotal*100.0)+ "% Hit: Video segment found in #{video} [#{realFrame}~#{realFrame+@@lengthSec}]\n")
        hitCounter += 1
        ...
        print(" TaniL:", "%2.5f" % taniLuma[x] , " TaniCU:", "%2.5f" % taniChromaU[x] , " TaniCV:", "%2.5f" % taniChromaV[x])
        print(" Sc:", "%5.2f" % (scoreTotal*100.0),"\n")
		#sleepT += 1
		#sleep += 1
      end
	  
	  if scoreTotal >= 0.85 && scoreP != scoreTotal && CM && !shall_we_norm #used to be 0.85
		print("O") if gui
        finalOut=finalOut+"%5.2f" % (scoreTotal*100.0)+ "% Hit: Video segment found in #{video} [#{realFrame}~#{realFrame+@@lengthSec}]\n"
		puts("%5.2f" % (scoreTotal*100.0)+ "% Hit: Video segment found in #{video} [#{realFrame}~#{realFrame+@@lengthSec}]\n")
        hitCounter += 1
	  ...
	  if scoreTotal >= 0.60 && scoreP != scoreTotal && CM && shall_we_norm
		print("O") if gui
        finalOut=finalOut+"%5.2f" % (scoreTotal*100.0)+ "% Hit: Video segment found in #{video} [#{realFrame}~#{realFrame+@@lengthSec}]\n"
		puts("%5.2f" % (scoreTotal*100.0)+ "% Hit: Video segment found in #{video} [#{realFrame}~#{realFrame+@@lengthSec}]\n")
        hitCounter += 1
...
puts("No hits were found! :(") if hitCounter == 0 && !sqlite_import_sql
puts("")
puts(finalOut)
