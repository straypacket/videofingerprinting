...
db_temp = SQLite3::Database.new( "/home/gsc/test_suj_branch2_central.db" ) if import
db = SQLite3::Database.new( "/home/gsc/test_suj_branch2_central_#{@@param}.db" )
#
...
videoArray.each do |video|
	...
	(0..(lumaArraySecCluster.keys.size - 1)).each do |idx|
		...
		mArray = Array.new
		mArray = lumaArrayFrames[(firstSec*fps).round..(firstSec*fps).round+@@lengthSec*fps.to_i]
		...
		if ( lumaArrayFrames.size < mArray.size )
			...
			tscore = vectD(searchSeqArrayLumaIni, mArray[0..bound])
			#tscore = tanimoto(searchSeqArrayLumaIni, mArray[0..bound])
		elsif ( lumaArrayFrames.size > mArray.size )
			bound = mArray.size-1
			tscore = vectD(searchSeqArrayLumaIni[0..bound], mArray)
		...
		#if tscore > 250
		#original value for tscore was 0.97
		if tscore < 0.995
			puts "[L3] Skipping MOVIE #{video}" if debug_stats || debug
			next
		end
		...
		p "[#{video}] Further analysing segment #{(lumaArraySecCluster.keys.sort[idx].to_f*fps).round}..#{(lumaArraySecCluster.keys.sort[idx+1].to_f*fps).round-1}" if debug || debug_stats
		((lumaArraySecCluster.keys.sort[idx].to_f*fps).round..(lumaArraySecCluster.keys.sort[idx+1].to_f*fps).round-1).each do |x|
			...
			##
			#Tanimoto
			taniLuma[x] = 0#tanimoto(searchSeqArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min], tempArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min])
    
			##
			#Difference between CLIP and MOVIE
			auxDiffLuma[x] = 0#diff(searchSeqArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min],tempArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min])
