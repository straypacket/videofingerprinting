...
#=begin
  ...
  (0..idxDistTimeLMovie.size-1).each do |x|
	...
	if count >= idxDistTimeLClip.size-4 && countL >= idxDistTimeLClip.size-5
	  b_val1 = true
	  #p("#{video} going to break out of level1")
	  break
	end
  end
...
=begin 
  ## level 2
  if b_val1 == true #&& toSearchL.empty?
    #p("#{video} got into level 2")
	...
	#p (idxDistTimeLClip & toSearchL[mid]).sort
	#p idxDistTimeLClip.uniq.sort
	if (idxDistTimeLClip & toSearchL[mid]).sort == idxDistTimeLClip.uniq.sort
	  b_val2 = true
	  #p("#{video} going to break out of level2")
	end
...
=end
  ...
  if b_val1 == false
  #if b_val2 == false #|| (idxDistTimeLClip.uniq.sort & toSearchL[mid].uniq.sort) != idxDistTimeLClip.uniq.sort #|| (idxDistTimeCvClip.uniq.sort & toSearchCv[mid].uniq.sort) != idxDistTimeCvClip.uniq.sort
    #break out of the movie loop, going for the next movie
	#p "#{video} is not good. Next!"
    next
  end
  
  #p "#{video} is a potential winner!"
