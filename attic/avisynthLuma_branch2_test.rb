#Usage
#avisynthLuma sourceVideo DBthreshold start length {-debug}
...
db_temp = SQLite3::Database.new( "/home/gsc/test_suj_branch2_temp.db" ) if import
db = SQLite3::Database.new( "/home/gsc/test_suj_branch2_#{@@param}.db" )
#
...
videoArray.each do |video|
	...
	if import  
		...
		begin
			...
			begin
				...
				db.execute2("create table \"#{video}\" (s_end FLOAT, luma INTEGER, chromau INTEGER, chromav INTEGER)")
				(0..keys.size-2).each do |x| 
					...
					#insert in DB
					db.execute( "insert into \"#{video}\" (s_end,luma,chromau,chromav) values (#{keys[x]/fps},#{avgLuma},#{avgChromaU},#{avgChromaV})")
					...
					#Luis' idea
					db.execute( "update hashcu set movies = \"#{prev.to_s+mid.to_s+":"+(((keys[x]/fps)*100).round/100.0).to_s+"-"+(((keys[x+1]/fps)*100).round/100.0).to_s+","}\" where avg_range = \"#{(avgChromaU/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
					...
					#Luis' idea
					db.execute( "update hashcv set movies = \"#{prev.to_s+mid.to_s+":"+(((keys[x]/fps)*100).round/100.0).to_s+"-"+(((keys[x+1]/fps)*100).round/100.0).to_s+","}\" where avg_range = \"#{(avgChromaV/100.0).round}\"" ) if prev.to_s.split(',').index(mid.to_s) == nil
			...
			#insert last element
			db.execute( "insert into \"#{video}\" (s_end,luma,chromau,chromav) values (#{keys[keys.size-1]/fps},-1,-1,-1)")
		end
		db.execute("END")
	...
	if toSearchL[mid] == nil && toSearchL.empty? == false
		p "Skipping #{video} ..." if debug
		next
	end
	...
	if sourceVideo == video
		...
		(0..auxClipArray.size-2).each do |d|
			idxDistTimeLClip << ((auxClipArray[d+1][0]-auxClipArray[d][0])*fps).round
			idxDistLumaClip << (auxClipArray[d+1][1]-auxClipArray[d][1]).round
		end
	...
	(1..auxArray.size-2).each do |d|
		idxDistTimeLMovie << ((auxArray[d+1][0]-auxArray[d][0])*fps).round
		idxDistLumaMovie << (auxArray[d+1][1]-auxArray[d][1]).round
	end
	...
	if toSearchL.empty?
		...
		auxClipArray = arrayCluster(searchSeqArrayLumaIni,@@param).sort
		#p auxClipArray
		auxClipArray.each do |avg|
			next if avg[1] < 0
	...
	if toSearchL[mid] == nil
		puts "Skipping MOVIE #{video}, as it has no segments related to the CLIP"
		next
	end
	...
	while c < tarray.size-1 do
		...
		#p("Diff between tarray.sort[#{c}] and tarray.sort[#{c}] = #{diff} Secs of contiguous info[#{tarray.sort[seqIdx]}~#{tarray.sort[c]}]:#{contSecs}") #if debug
		break if contSecs >= @@lengthSec
	end
	#end
	
	#p tarray.sort #if contSecs >= @@lengthSec*0.4
	#p "#{video} has a sum of #{sum} and contSecs of #{contSecs}" #if contSecs >= @@lengthSec*0.4
	
	b_val1 = true if sum >= @@lengthSec*0.4
	
	next if b_val1 == false
		...
		(0..idxDistTimeLMovie.size-1).each do |x|
			...
			if x-1 <= idxDistTimeLMovie.size-idxDistTimeLClip.size
				...
				(0..idxDistTimeLClip.size-1).each do |y|
					...
					countL += 1 if b_array[y] >= idxDistLumaClip[y]-b_thresh && b_array[y] <= idxDistLumaClip[y]+b_thresh
				end
			end

			#p "L  #{video}: idx=#{x} sec:#{auxArray[x][0]} Count=#{count}/#{idxDistTimeLClip.size-1} CountL=#{countL}/#{idxDistLumaClip.size-1}"
			#p "#{video}: idx=#{x} Count=#{count}/#{idxDistTimeLClip.size-1} CountL=#{countL}/#{idxDistTimeLClip.size-1}"

			if count > 0 && countL > 0
				if idxDistTimeLClip.size < 10
					if count >= idxDistTimeLClip.size-4 && countL >= idxDistLumaClip.size-3
					...
					if count >= (idxDistTimeLClip.size)*0.6 && countL >= (idxDistLumaClip.size)*0.9
						p "#{video}: idx=#{x} sec:#{auxArray[x][0]} Count=#{count}/#{idxDistTimeLClip.size} CountL=#{countL}/#{idxDistLumaClip.size}" if debug
						b_val2 = true
						break
					end
	...
	next if b_val2 == false 
	
	#p "#{video} passed to phase 2"
	...
	(0..(lumaArraySecCluster.keys.size - 1)).each do |idx|
		...
		#chunk of MOVIE array
		mArray = Array.new
		mArray = lumaArrayFrames[(lumaArraySecCluster.keys.sort[idx].to_f*fps).round..(lumaArraySecCluster.keys.sort[idx].to_f*fps).round+@@lengthSec*fps.to_i]
		#p searchSeqArrayLumaIni
		#p mArray
		
		if ( lumaArrayFrames.size < mArray.size )
			bound = lumaArrayFrames.size-1
			tscore = tanimoto(searchSeqArrayLumaIni, mArray[0..bound])
		elsif ( lumaArrayFrames.size > mArray.size )
			bound = mArray.size-1
		...
		#p mArray
		if debug
			p "tanimoto for #{lumaArraySecCluster.keys.sort[idx]}(#{lumaArraySecCluster.keys.sort[idx]*fps.round})~#{lumaArraySecCluster.keys.sort[idx]+@@lengthSec}(#{(lumaArraySecCluster.keys.sort[idx]+@@lengthSec)*fps.round}): #{tscore}"
		end
		...
		p "[#{video}] Further analysing segment #{(lumaArraySecCluster.keys.sort[idx].to_f*fps).round}..#{(lumaArraySecCluster.keys.sort[idx+1].to_f*fps).round-1}" if debug
		((lumaArraySecCluster.keys.sort[idx].to_f*fps).round..(lumaArraySecCluster.keys.sort[idx+1].to_f*fps).round-1).each do |x|
			...
			#p searchSeqArrayLuma.size
			#p tempArrayLuma.size
			##
			#Tanimoto
			taniLuma[x] = tanimoto(searchSeqArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min], tempArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min])
    
			##
			#Difference between CLIP and MOVIE
			auxDiffLuma[x] = diff(searchSeqArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min],tempArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min])
    
			##
			#Distance between vectors
			auxSqrtLuma[x] = vectD(searchSeqArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min],tempArrayLuma[0..[searchSeqArrayLuma.size-1,tempArrayLuma.size-1].min])
			
			#0.995 2 60
			if taniLuma[x] > 0.97 && auxDiffLuma[x] < 5 && auxSqrtLuma[x] < 350
				print(video, ": ", x, " LDiff:", "%2.5f" % auxDiffLuma[x], " LVectD:", "%2.5f" % auxSqrtLuma[x])
				print(" TaniL:", "%2.5f" % taniLuma[x] )
				print("\n")
