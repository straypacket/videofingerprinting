...
if sqlite
  ...
  db = SQLite3::Database.new( "/home/gsc/test_3tablepermovie_#{@@param}.db" ) if linux && threeTables && ( sqlite || sqlite_import )
  #
  begin
    if sqlite_import
...
idxDistClip = Array.new
idxDistLumaClip = Array.new
idxDistMovie = Array.new
...
#=begin
    ...
    (0..test.size-2).each do |d|
      #p "Adding Clip luma diff #{(auxClipArray[d+1][1]-auxClipArray[d][1]).abs} and time diff #{auxClipArray[d+1][0]-auxClipArray[d][0]} for idx #{auxClipArray[d][0]}"
      
      idxDistClip << auxClipArray[d+1][0]-auxClipArray[d][0]
      idxDistLumaClip << (auxClipArray[d+1][1]-auxClipArray[d][1]).abs
    end
    idxDistClip << auxClipArray[auxClipArray.size-1][0]-auxClipArray[auxClipArray.size-2][0]
...
#=end
  ...
  (0..auxArray.size-2).each do |d|
    #p "Adding Movie luma diff #{(auxArray[d+1][1]-auxArray[d][1]).abs/100} and time diff #{auxArray[d+1][0]-auxArray[d][0]} for idx #{auxArray[d][0]}"
    
    idxDistMovie << auxArray[d+1][0]-auxArray[d][0]
    idxDistLumaMovie << (auxArray[d+1][1]-auxArray[d][1]).abs/100 
  end
  idxDistMovie << auxArray[auxArray.size-1][0]-auxArray[auxArray.size-2][0]
  idxDistLumaMovie << (auxArray[auxArray.size-1][1]-auxArray[auxArray.size-2][1]).abs/100
...
#=begin
  ...
  (0..idxDistMovie.size-1).each do |x|
    count = 0
    countL = 0
    if x-1 <= idxDistMovie.size-idxDistClip.size
      #b_array = norm(idxDistLumaMovie[x..x+idxDistClip.size-1])
      b_array = idxDistLumaMovie[x..x+idxDistClip.size-1]
      (0..idxDistClip.size-2).each do |y|
        #p " T= #{video}: sec:#{auxArray[x+y][0]} idx:#{x} #{idxDistMovie[x+y]} == #{idxDistClip[y]}?  --  #{(b_array[y])} ~= #{(idxDistLumaClip[y])}?"
        count +=1 if idxDistMovie[x+y] == idxDistClip[y]
        #p " L= #{video}: #{(idxDistLumaMovie[x+y]*100).round} >= #{(idxDistLumaClip[y]*100).round*(1-b_thresh)} && #{(idxDistLumaMovie[x+y]*100).round} <= #{(idxDistLumaClip[y]*100).round*(1+b_thresh)}?"
        countL += 1 if b_array[y] >= idxDistLumaClip[y]*(1-b_thresh) && b_array[y] <= idxDistLumaClip[y]*(1+b_thresh)
      end

      #p "#{video}: idx=#{x} sec:#{auxArray[x][0]} Count=#{count}/#{idxDistClip.size-1} CountL=#{countL}/#{idxDistClip.size-1}"
    
    end

    if count >= idxDistClip.size-4 && countL >= idxDistClip.size-5
      #p "#{video}: idx=#{x} Count=#{count}/#{idxDistClip.size-1} CountL=#{countL}/#{idxDistClip.size-1}"
      b_val = true
      break
    end
