...
#=begin  
  ...
  if toSearchL.empty?
	...
	auxArray.each do |avg|
		...
		db.execute( "select * from hashluma where avg_range = #{((avg[1].to_i/100.0).round)+0}" )[0][1].to_s.split(',').sort.uniq.each do |tuple|
			#p aaux
...
#=begin
  #if true
  if toSearchL.has_key?(mid) == true && toSearchCu.has_key?(mid) == true && toSearchCv.has_key?(mid) == true
...
#		end
		...
		if count >= idxDistTimeLClip.size-4 && countL >= idxDistTimeLClip.size-6
...
#=end
  ...
  if b_val == false || (idxDistTimeLClip.uniq.sort & toSearchL[mid].uniq.sort) != idxDistTimeLClip.uniq.sort #|| (idxDistTimeCvClip.uniq.sort & toSearchCv[mid].uniq.sort) != idxDistTimeCvClip.uniq.sort
