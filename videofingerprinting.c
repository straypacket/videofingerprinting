...
int cgo(AVFrame *pFrame, int width, int height, int iFrame, char *argv[], sqlite3* handle) {
  int y = 1;
  int x = 1;
...
//  avgChromaV = chromaV*1.0 / ((height*width)/N);
  ...
  if ( (iFrame%max_frames) == 0 ) {
	...
	split++;
	if (split != 0) {
		sprintf(table_query,"create table '%s_%d' (s_end INTEGER, luma INTEGER);",argv[2],split);
		sqlite3_exec(handle,table_query,0,0,0);
		sprintf(table_query,"insert into allmovies (name,fps) values ('%s_%d',%f);",argv[2],split,fps);
		sqlite3_exec(handle,table_query,0,0,0);
	}
  ...
  //printf("%d;time;%f;%f;%f;%f\n", iFrame, avgLuma, fabs(avgLuma-prevY), avgChromaU, avgChromaV);
  if (split == 0) {
	sprintf(insert_query, "insert into '%s' values (%d,%f)",argv[2],iFrame,avgLuma);
  }
  else {
    sprintf(insert_query, "insert into '%s_%d' values (%d,%f)",argv[2],split,iFrame-split_second,avgLuma);
  }
