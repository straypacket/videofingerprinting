...
To compile:
Linux
gcc -o videofingerprinting videofringerprinting.c -lsqlite3 -lfftw3 -lavutil -lavformat -lavcodec -lswscale -lz -lm -Wall
OSX
...
int AvgFrameImport(AVFrame *pVideoFrameFoo, int width, int height, int iFrame, char *filename, sqlite3 *handle, double fps, int *fullArray);
...
int AvgFrameImport(AVFrame *pVideoFrameFoo, int width, int height, int iFrame, char *filename, sqlite3* handle, double fps, int *fullArray) {
  ...
  for(y=height/N; y<2*height/N; y++)
    for (x=width/M; x<2*width/M; x++)
      luma += pVideoFrameFoo->data[0][y*pVideoFrameFoo->linesize[0] + x];
