...
(3..ARGV.size-1).each do |arg|
  ...
  elsif ARGV[arg] == "-hashparam"
    @@hash_param = ARGV[arg+1].to_i
  elsif ARGV[arg] == "-l3thresh"
    @@l3fact = ARGV[arg+1].to_f
  elsif ARGV[arg] == "-l2thresh"
    @@bthresh = ARGV[arg+1].to_i
  elsif ARGV[arg] == "-import"
    puts("SQLite import from temp database ON")
    import = true
  elsif ARGV[arg] == "-no_orig"
    puts("Not showing the original first match")
    no_orig = true
...
if robust
	db_orig = SQLite3::Database.new( "/Users/gsc/vfp_1250_fixed.db" )
	db = SQLite3::Database.new( "/Users/gsc/vfp_1250_full_#{@@robust_param}.db" )
else
	db = SQLite3::Database.new( "/Users/gsc/vfp_1250_fixed.db" )
end

#Re-initializing videoArray to contain all previously scanned movies by the .c program and sucessfuly inserted in the database
if import 
...
videoArray.each do |video|
	...
	if toSearchL.empty?
		...
		arrayCluster(searchSeqArrayLumaIni,@@param).sort.each do |avg|
			...
			qArray = db.execute( "select * from hashluma where avg_range between #{((avg[1].to_f).round)-(@@hash_param*1.0)} and #{((avg[1].to_f).round)+(@@hash_param*1.0)}" ) if (@@hash_param*1.0) >= 1
			qArray = db.execute( "select * from hashluma where avg_range = #{((avg[1].to_f).round)}" ) if (@@hash_param*1.0) == 0
	...
	#
	#for qcif anf 5fps:
	b_thresh = (@@bthresh*1.0)/10.0
	#b_thresh = 0.1
	#for qcif anf 5fps:
	b_thresh_L = (@@bthresh*1.0)
	#b_thresh_L = 1.0
	found_idx.clear
	if b_val1 == true
...
#=end
			...
			if countG >= idxDistLumaClip.size
				puts "#{video}: L: idx=#{x} sec:#{auxArray[x][0]} CountG=#{countG}/#{idxDistLumaClip.size}" if debug || debug_stats
				found_idx << x
				b_val2 = true
			end
...
=end
	...
	(0..(lumaArraySecCluster.keys.size - 1)).each do |idx|
		...
		##((lumaArraySecCluster.keys.sort[idx].to_f*fps).round..(lumaArraySecCluster.keys.sort[idx+1].to_f*fps).round).each do |x|
			...
			if auxSqrtLuma[x] < (340/@@l3fact) && taniLuma[x] > (0.9*@@l3fact) #&& auxDiffLuma[x] < 2 
				if auxSqrtLuma[x] < old_v || taniLuma[x] > old_t
					if taniLuma[x] > old_t || auxSqrtLuma[x] < old_v
						...
						line = sprintf("%s [%d]: f=%d t=%.2f LVectD:%2.5f TaniL:%2.5f fps=%d\n", video, gcount, x, x/fps, auxSqrtLuma[x], taniLuma[x], fps)
	...
	#gcount=0 is the original, first match
	gcount += 1
...
print("Hits found: #{hitCounter} Unique hits found: #{uniqCounter.uniq.size}\n")
