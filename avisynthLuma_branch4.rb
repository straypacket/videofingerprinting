...
videoArray.each do |video|
	...
	if import  
		...
		if rows > 0
			(0..rows-1).each do |f|
				...
				lumaArraySec[query[f][0].to_i] = query[f][0].to_i
	...
	if robust && searchSeqArrayLumaIni.size == 0 && searchSeqArrayTimeIni.size == 0&& sourceVideo == video
    ...
    begin
		if robust && searchSeqArrayLumaIni.size == 0 && searchSeqArrayTimeIni.size == 0
	...
	if sourceVideo == video && searchSeqArrayLumaIni.size == 0 && searchSeqArrayTimeIni.size == 0
		aux = 0
		...
		if (lumaArrayFrames.size-1)/fps >= firstSec+@@lengthSec
			((firstSec*fps).round..((firstSec*fps).round+((@@lengthSec*fps)-1).round)).each do |x|
				...
				searchSeqArrayTimeIni[aux] = auxClipArray.rassoc(lumaArrayFrames[x])[0]
		...
		if debug
			...
			puts "Post-clustered time (searchSeqArrayTimeIni.uniq):"
	...
	(0..(lumaArraySecCluster.keys.size - 1)).each do |idx|
		...
		while ((auxArray[n][0].to_f) <= ((auxArray[idx][0].to_f)+@@lengthSec))
			if robust
				(idxDistTimeLMovie[n]*ofps).round.times { mArrayL << auxArray[n][1].to_f}
				(idxDistTimeLMovie[n]*ofps).round.times { mArrayT << auxArray[n][0].to_f}
			else
		...
		if l3uniq
			if ( searchSeqArrayLumaIni.uniq.size < mArrayL.uniq.size )
				bound = searchSeqArrayLumaIni.size-1
				tscoreL = tanimoto(searchSeqArrayLumaIni.uniq, mArrayL[0..bound].uniq)
				
				p "--" if debug
				p searchSeqArrayLumaIni.uniq if debug
				p "-" if debug
				p mArrayL[0..bound].uniq if debug
				p "--" if debug
			elsif ( searchSeqArrayLumaIni.uniq.size == mArrayL.uniq.size )
				tscoreL = tanimoto(searchSeqArrayLumaIni.uniq, mArrayL.uniq)
				
				p "--" if debug
				p searchSeqArrayLumaIni.uniq if debug
				p "-" if debug
				p mArrayL.uniq if debug
				p "--" if debug
			else #( searchSeqArrayLumaIni.size > mArrayL.size )
				bound = mArrayL.size-1
				tscoreL = tanimoto(searchSeqArrayLumaIni[0..bound].uniq, mArrayL.uniq)
				
				p "--" if debug
				p searchSeqArrayLumaIni[0..bound].uniq if debug
				p "-" if debug
				p mArrayL.uniq if debug
				p "--" if debug
			end
		...
		if l3uniq
			if ( searchSeqArrayTimeIni.uniq.size < mArrayT.uniq.size )
				bound = searchSeqArrayTimeIni.size-1
				tscoreT = tanimoto(searchSeqArrayTimeIni.uniq, mArrayT[0..bound].uniq)
				
				p "--" if debug
				p searchSeqArrayTimeIni.uniq if debug
				p "-" if debug
				p mArrayT[0..bound].uniq if debug
				p "--" if debug
			elsif ( searchSeqArrayTimeIni.uniq.size == mArrayT.uniq.size )
				tscoreT = tanimoto(searchSeqArrayTimeIni.uniq, mArrayT.uniq)
				
				p "--" if debug
				p searchSeqArrayTimeIni.uniq if debug
				p "-" if debug
				p mArrayT.uniq if debug
				p "--" if debug
			else #( searchSeqArrayTimeIni.size > mArrayT.size )
				bound = mArrayT.size-1
				tscoreT = tanimoto(searchSeqArrayTimeIni[0..bound].uniq, mArrayT.uniq)
				
				p "--" if debug
				p searchSeqArrayTimeIni[0..bound].uniq if debug
				p "-" if debug
				p mArrayT.uniq if debug
				p "--" if debug
			end
