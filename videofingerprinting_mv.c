...
int MVFrameImport(AVFrame *pVideoFrameFoo, int width, int height, int frame, int *modLuma, int *prevLuma);
...
int MVFrameImport(AVFrame *pVideoFrameFoo, int width, int height, int frame, int *modLuma, int *prevLuma) {
  ...
  for(y=0; y<height; y++){
    for (x=0; x<width; x++){
      modLuma[x+(y*width)] += abs(prevLuma[x+(y*width)] - pVideoFrameFoo->data[0][y*pVideoFrameFoo->linesize[0] + x]);
	  prevLuma[x+(y*width)] = pVideoFrameFoo->data[0][y*pVideoFrameFoo->linesize[0] + x];
    }
  }
